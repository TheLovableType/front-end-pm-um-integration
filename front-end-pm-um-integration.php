<?php
/*
Plugin Name: Front End PM - UM Integration
Plugin URI: https://www.shamimsplugins.com/contact-us/
Description: Front End PM custom extension to integrate with Ultimate Member
Version: 1.1
Author: Shamim
...
class Front_End_Pm_UM_Integration {
	...
	function content( $args ) {
		if( get_current_user_id() == um_profile_id() ){
			...
			#fep-header .avatar {display: inline-block;}
			</style>
			<?php
	...
	function account_tab( $tabs ) {
			...
			$tabs[800]['fep_um_message_tab']['title'] = __('Message', 'front-end-pm');
	...
	function account_content( $output ){
			...
			$output .= '<a class="um-button" href="' . add_query_arg( 'profiletab', 'fep_um_message_tab', um_user_profile_url()) .'">' . __('Message', 'front-end-pm') . '</a>';
			$output .= '</div>';
	...
	public function notices() {
		echo '<div class="error"><p>'. __( 'Front End PM and Ultimate Member must be activated to use Front End PM UM Integration extension.', 'front-end-pm' ). '</p></div>';
	}
